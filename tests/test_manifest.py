...
def test_load_legacy_v1_manifest_migrates_to_multichain(tmp_path):
    path = tmp_path / ".lineage-wiki" / "manifest.yml"
    path.parent.mkdir()
    path.write_text(
        """
version: 1
...
def test_load_legacy_v1_manifest_rejects_empty_chain_id(tmp_path):
    path = tmp_path / ".lineage-wiki" / "manifest.yml"
    path.parent.mkdir()
    path.write_text(
        """
...
""".lstrip(),
    ...
    with pytest.raises(ValueError, match="empty `chain_id`"):
...
def test_equality_ignores_run_time():
    base = ChainManifest(chain_slug="x", last_content_snapshot="sha256:s")
    m1 = Manifest(
        chains={
            "x": base.model_copy(
                update={"last_run_at": "2026-07-03T00:00:00Z"}
            )
        }
    )
    m2 = Manifest(
        chains={
            "x": base.model_copy(
                update={"last_run_at": "2026-07-04T00:00:00Z"}
            )
        }
    )
    ...
    m3 = Manifest(
        chains={
            "x": base.model_copy(update={"last_content_snapshot": "sha256:other"})
        }
    )
